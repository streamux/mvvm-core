 ...
 * Refactoring
 *
 * 1. internalize modules into base class : done
 *
 **/
...
const componentParser = (temp, scope) => {
  const events = {};
  let components = temp;
...
class EventManager {
  ...
  addEvent(type, listener) {
    if (this.events.find(e => e.type === type && e.listener === listener) && isNativeEvent(type)) return;
...
class BaseComponent {
  ...
  render() {
    const { components, emitEvent } = componentParser(this.template(), this);
